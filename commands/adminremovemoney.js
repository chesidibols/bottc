...
module.exports.run = async (bot, message, args) =>{
    ...
    Data.findOne({
        ...
        message.channel.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:coinns:715103658601218088> to **${bot.users.cache.get(user.id).tag}**`}});
        logsCoin.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:coinns:715103658601218088> to **${bot.users.cache.get(user.id).tag}**`}});
        admin.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:coinns:715103658601218088> to **${bot.users.cache.get(user.id).tag}**`}});
        return;

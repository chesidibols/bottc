...
module.exports.run = async (bot, message, args) =>{
    ...
    Data.findOne({
        ...
        message.channel.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:coinns:699944502856646716> to **${bot.users.cache.get(user.id).tag} and has ${money} <:coinns:699944502856646716>**`}});
        logsCoin.send({embed:{color:'a20a28', description:`**${message.author.username}** removed ${args[1]}<:coinns:699944502856646716> to **${bot.users.cache.get(user.id).tag} and has ${money} <:coinns:699944502856646716>**`}});
        return;
